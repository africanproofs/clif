...
- **D20 — `NETWORK` selects the chain (unset = silent flare); the v2 bundle must carry it; `import-credentials` auto-seeds nonces (2026-06-12; fwd a103–a106 lockstep).** clif resolves the active chain as `net = network or os.environ.get("NETWORK") or "flare"` (`clif/cli.py:715`), so a `.env.<network>` (or the v2 bundle `config`) **without a `NETWORK` key makes clif silently default to flare** — for `preflight` AND the `epoch run` daemon, i.e. a songbird deployment would sign/claim on the **wrong chain** with no error. Surfaced when `clifctl run songbird preflight` printed `Preflight — flare (chain 14)` after a fresh v2-bundle onboard (D19) that had dropped `NETWORK` on the fwd side (note: `clifctl nonce-sync` got chain 19 right because the *wrapper* maps net→chain itself — only the in-container clif command reads `NETWORK`). **(a) Fix is fwd-side + allowlist-clean:** fwd's `install/onboard` now composes `NETWORK` into the bundle `config` (fwd a105); clif accepts it because `config_env_allowlist` (D19) is derived from `Settings.network`, so no clif code change was needed — only this contract pin: **`.env.<network>` MUST carry `NETWORK`** (from the bundle `config`) or the command must pass `--network`. **(b) Auto-seed:** `clifctl import-credentials` now auto-runs `nonce-sync` after a successful import (best-effort, only when `clifwd` is co-located) — membrane-clean: clif reads the on-chain tx-count (egress), `clifwd` writes fwd's nonce (fwd is zero-egress), so a co-located onboard needs no separate `nonce-sync`. *Why: the v2 "complete handoff" (D19) is only complete if it carries the network selector; a default-masked omission is invisible on the production (flare) chain and only a non-default (songbird) canary exposes it.*
