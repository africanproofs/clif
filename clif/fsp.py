    ...
    *,
    ...
    if rpc is not None:
        try:
            ...
            log.warning("fsp uptime pre-flight finalization check failed (proceeding): %s", exc)
    ...
    with (
        ...
        log.info(
            ...
            mt,
            reward_epoch_id,
            settings.fsp_signing_wallet_name,
            leg1_key,
        )
        try:
            ...
            return _out(
                mt, reward_epoch_id, OutcomeStatus.FAILED_TERMINAL, f"fwd leg-1 denied: {exc}{hint}"
            )
        except FwdRetryableError as exc:
            return _out(
                mt, reward_epoch_id, OutcomeStatus.FAILED_RETRYABLE, f"fwd leg-1 transient: {exc}"
            )
        ...
        log.info(
            ...
            mt,
            reward_epoch_id,
            settings.fsp_sender_wallet_name,
...
def run_sign_rewards(
    ...
    *,
    ...
    log.info(
        "fsp rdd epoch-bound epoch=%s (file rewardEpochId==signing epoch) " "merkle_root=%s n=%s",
        reward_epoch_id,
        rdd.merkle_root,
    ...
    if rpc is not None:
        try:
            ...
            log.warning("fsp rewards pre-flight finalization check failed (proceeding): %s", exc)
